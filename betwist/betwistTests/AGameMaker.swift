...
struct AGameMaker {
  ...
  func returns_enough_letters() {
    #expect(GameGenerator(3).make().count == 9)
    #expect(GameGenerator(4).make().count == 16)
    #expect(GameGenerator(5).make().count == 25)
  }

  @Test
  func returns_lots_of_e_compared_to_k() {
    let sample = GameGenerator(15).make()
  ...
  func converts_q_to_qu() {
    let sample = GameGenerator(1, ["QQQQQQ"]).make()
