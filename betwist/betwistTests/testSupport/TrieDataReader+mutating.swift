...
extension TrieDataReader {
  ...
  mutating func appendAddress(_ offset: Int) {
    let byte0 = UInt8((offset >> 16) & 0xff)
    let byte1 = UInt8((offset >> 8) & 0xff)
    let byte2 = UInt8(offset & 0xff)
    append(bytes: [byte0, byte1, byte2])
  }
